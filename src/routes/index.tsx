      ...
      <header className={`sticky top-0 z-40 transition-all ${scrolled ? "bg-background/85 backdrop-blur border-b border-border" : "bg-background/60 backdrop-blur-sm"}`}>
        <div className="mx-auto max-w-7xl px-4 sm:px-6 py-3 flex items-center justify-between gap-4">
          ...
          <nav className="hidden lg:flex items-center gap-7 text-sm font-medium">
            {navLinks.map(l => (
              <a key={l.href} href={l.href} className="text-foreground/75 hover:text-primary transition-colors">{l.label}</a>
            ))}
          </nav>

          <div className="flex items-center gap-2">
            <a href="#donate" className="hidden sm:inline-flex items-center gap-1.5 rounded-full bg-accent px-4 py-2 text-sm font-semibold text-accent-foreground hover:brightness-95 transition">
        ...
        {menuOpen && (
          <div className="lg:hidden border-t border-border bg-background">
            <div className="px-4 py-3 flex flex-col">
              {navLinks.map(l => (
                <a key={l.href} href={l.href} onClick={()=>setMenuOpen(false)} className="py-2.5 text-sm font-medium border-b border-border/60 last:border-0">{l.label}</a>
              ))}
            </div>
          </div>
        )}
