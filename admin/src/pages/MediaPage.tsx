...
function resolveImageUrl(imagePath?: string | null) {
  ...
  return `${uploadsBaseUrl}/uploads/medias/${imagePath}`;
}
...
export default function MediaPage() {
  ...
  const mediaCategories = useMemo(
    () =>
      categories
        .filter((category) => String(category.type || '').toLowerCase() === 'media')
  ...
  const stats = useMemo(
    () => ({
      ...
      types: new Set(data.map((item) => item.type).filter(Boolean)).size,
    }),
    [data],
  );
            ...
            <div className="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
              <div className="max-w-2xl">
                <div className="mb-4 inline-flex items-center gap-3 rounded-full border border-white/15 bg-white/10 px-4 py-2">
                  ...
                  <span className="text-sm font-semibold tracking-[0.2em] uppercase text-white/90">Media Management</span>
                </div>
                <h1 className="text-3xl font-bold tracking-tight md:text-4xl">Media</h1>
              </div>
                ...
                <table className="w-full min-w-[1320px] border-collapse">
                  <thead>
                    <tr className="bg-[linear-gradient(180deg,#f5fbf7_0%,#edf5ef_100%)] text-left">
                      {['ID', 'Image', 'Title', 'Category', 'Type', 'Publisher', 'Published Date', 'Status', 'Actions'].map((label) => (
                  ...
                  <tbody>
                    {loading ? (
                      <tr><td colSpan={9} className="px-6 py-12 text-center text-sm text-[#6d8377]">Loading media...</td></tr>
                    ) : displayRows.length === 0 ? (
                      <tr><td colSpan={9} className="px-6 py-14 text-center text-sm text-[#6d8377]">No media records found.</td></tr>
                    ) : (
                      paginatedRows.map((item) => (
                        <tr key={item.id} className="border-t border-[#edf2ee] text-sm text-[#2d4a3c] hover:bg-[#fbfdfb]">
                          ...
                          <td className="px-4 py-4">{item.image ? <img src={resolveImageUrl(item.image)} alt={item.title || 'Media image'} className="h-16 w-24 rounded-xl border border-[#dfe9e3] bg-white object-cover" /> : 'No image'}</td>
                          ...
                          <td className="px-4 py-4">{item.publisher_name || '-'}</td>
                          <td className="px-4 py-4">{item.published_date || '-'}</td>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#0b1e15]/60 p-4 backdrop-blur-sm">
          <div className="w-full max-w-sm rounded-[28px] border border-[#d7e4db] bg-white p-6 shadow-[0_35px_80px_rgba(8,30,20,0.28)]">
            ...
            <h3 className="text-lg font-semibold text-[#1a4731]">Delete Media</h3>
            <p className="mt-2 text-sm leading-6 text-[#61766a]">This will remove <span className="font-semibold text-[#1f3f2f]">{deleteTarget.title}</span> from the medias table.</p>
