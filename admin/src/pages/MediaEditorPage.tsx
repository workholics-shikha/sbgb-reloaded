...
import { ArrowLeft, Newspaper, Save, Upload } from 'lucide-react';
...
export default function MediaEditorPage() {
  ...
  const { data, loading, error, addItem, updateItem, uploadImage } = useMedias();
  ...
  useEffect(() => {
    ...
    if (isEditing) {
      if (!editingItem) {
        setFormError('Selected media item could not be found.');
        return;
      }

      setFormValues({
        category_id: editingItem.category_id ?? '',
        type: editingItem.type ?? DEFAULT_TYPE_OPTIONS[0],
        image: editingItem.image ?? '',
        title: editingItem.title ?? '',
        published_date: editingItem.published_date ?? '',
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setFormValues((current) => ({ ...current, type: e.target.value }))}
                  className="w-full rounded-2xl border border-[#d7e4db] bg-white px-4 py-3 text-sm text-[#1a4731] outline-none"
                >
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setFormValues((current) => ({ ...current, published_date: e.target.value }))}
                  className="w-full rounded-2xl border border-[#d7e4db] px-4 py-3 text-sm text-[#1a4731] outline-none"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setFormValues((current) => ({ ...current, publisher_name: e.target.value }))}
                  className="w-full rounded-2xl border border-[#d7e4db] px-4 py-3 text-sm text-[#1a4731] outline-none"
                />
              </div>
            </div>
            ...
            <div className="flex gap-3 pt-2">
              ...
              >
                <span className="inline-flex items-center gap-2">
                  <Save size={16} />
                  {saving ? 'Saving...' : isEditing ? 'Update Media' : 'Save Media'}
                </span>
              </button>
            </div>
          </form>
