...
import { ChevronLeft, ChevronRight, Eye, FileText, ImagePlus, Pencil, Trash2, Upload, X } from 'lucide-react';
...
export default function ArticlesPage() {
  const { data, loading, error, addItem, updateItem, deleteItem, uploadImage } = useArticles();
  ...
  const articleCategoryOptions = useMemo(
    () =>
      categories
        .filter((category) => String(category.type || '').toLowerCase() === 'article')
        .map((category) => ({
  ...
  const categoryMap = useMemo(
    () => new Map(articleCategoryOptions.map((category) => [category.value, category.label])),
    [articleCategoryOptions],
  );

  const defaultCategory = articleCategoryOptions[0];

  const stats = useMemo(() => {
    const publishedCount = data.filter((article) => article.is_published).length;
    const categoryCount = new Set(data.map((article) => article.category_id).filter(Boolean)).size;
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const selectedCategoryName = categoryMap.get(formValues.category_id) || '';
      ...
      const payload = {
        ...
        image: imagePath,
        article_date: formValues.article_date.trim(),
        article_owner: formValues.article_owner.trim(),
        description: formValues.description.trim(),
        status: Number(formValues.status),
      };

      if (editingArticle) {
        await updateItem(editingArticle.id, payload);
      ...
      setFormError(submitError instanceof Error ? submitError.message : 'Unable to save article');
    } finally {
      setSaving(false);
    }
  }
  ...
  async function handleDelete() {
    ...
    try {
      ...
      setFormError(deleteError instanceof Error ? deleteError.message : 'Unable to delete article');
    }
  }
            ...
            <div className="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
              <div className="max-w-2xl">
                ...
                <p className="mt-3 max-w-xl text-sm leading-6 text-white/80 md:text-base">
                  Showing the live `articles` table data from the `team_sbgbt` database on this page.
                </p>
              </div>
          ...
          <div className="p-5 md:p-7">
            ...
            <div className="mb-5 flex flex-col gap-4 rounded-[24px] border border-[#d9e6dd] bg-[linear-gradient(180deg,#fcfdfc_0%,#f4f8f5_100%)] p-4 shadow-[0_12px_32px_rgba(26,71,49,0.05)] md:flex-row md:items-center md:justify-between md:p-5">
              <div>
                ...
                <h2 className="mt-1 text-2xl font-semibold text-[#1a4731]">Articles</h2>
              </div>
                ...
                <table className="w-full min-w-[1340px] border-collapse">
                  <thead>
                    <tr className="bg-[linear-gradient(180deg,#f5fbf7_0%,#edf5ef_100%)] text-left">
                      ...
                      <th className="px-4 py-4 text-xs font-bold uppercase tracking-[0.18em] text-[#1a4731]">Owner</th>
                      <th className="px-4 py-4 text-xs font-bold uppercase tracking-[0.18em] text-[#1a4731]">Date</th>
                  ...
                  <tbody>
                    {loading ? (
                      ...
                      <tr>
                        <td colSpan={8} className="px-6 py-14 text-center">
                          <div className="mx-auto flex max-w-sm flex-col items-center">
                            ...
                            <p className="text-base font-semibold text-[#1a4731]">No articles found</p>
                            <p className="mt-1 text-sm text-[#6d8377]">
                              Add a new article record to start populating this table.
                            </p>
                          </div>
                        </td>
                      </tr>
                      ...
                      paginatedRows.map((article) => (
                        ...
                        >
                          <td className="px-6 py-4 font-semibold text-[#1f3f2f]">{article.id}</td>
                          ...
                          <td className="px-6 py-4">
                            <div className="max-w-[360px]">
                              <p className="font-semibold text-[#1f3f2f]">{article.title || 'Untitled article'}</p>
                            </div>
                          </td>
                          <td className="px-4 py-4 text-[#52685c]">
                            <div className="max-w-[220px] rounded-2xl border border-[#e4ece7] bg-[#f8fbf9] px-3 py-2 text-xs">
                              {article.category || categoryMap.get(article.category_id) || '-'}
                            </div>
                          </td>
                          <td className="px-4 py-4 text-[#52685c]">{article.article_owner || '-'}</td>
                          <td className="px-4 py-4 text-[#52685c]">{article.article_date || '-'}</td>
                          <td className="px-4 py-4">
                            <span
                              className={`inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs font-semibold ${
                                article.is_published ? 'bg-[#edf6ef] text-[#2e7d52]' : 'bg-[#fff2ef] text-[#d94b3d]'
                              }`}
                            >
          ...
          <div className="w-full max-w-5xl overflow-hidden rounded-[28px] border border-[#d7e4db] bg-white shadow-[0_35px_80px_rgba(8,30,20,0.3)]">
            <div className="bg-[linear-gradient(135deg,#1a4731_0%,#2e7d52_100%)] px-6 py-5 text-white">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#f0cd72]">Article Form</p>
            ...
            <form onSubmit={handleSubmit} className="space-y-5 px-6 py-6">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div>
                  <label className="mb-1.5 block text-sm font-medium text-[#456353]">Article Date</label>
                  <input
                    required
                    value={formValues.article_date}
                    onChange={(e) => setFormValues((current) => ({ ...current, article_date: e.target.value }))}
                    className="w-full rounded-2xl border border-[#d7e4db] px-4 py-3 text-sm text-[#1a4731] outline-none transition focus:border-[#2e7d52] focus:ring-4 focus:ring-[#2e7d52]/10"
                  />
                </div>

                <div>
                  <label className="mb-1.5 block text-sm font-medium text-[#456353]">Article Owner Name</label>
                  <input
                    required
                    value={formValues.article_owner}
                    onChange={(e) => setFormValues((current) => ({ ...current, article_owner: e.target.value }))}
                    className="w-full rounded-2xl border border-[#d7e4db] px-4 py-3 text-sm text-[#1a4731] outline-none transition focus:border-[#2e7d52] focus:ring-4 focus:ring-[#2e7d52]/10"
                  />
                </div>
              </div>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#0b1e15]/60 p-4 backdrop-blur-sm">
          <div className="w-full max-w-sm rounded-[28px] border border-[#d7e4db] bg-white p-6 shadow-[0_35px_80px_rgba(8,30,20,0.28)]">
            ...
            <h3 className="text-lg font-semibold text-[#1a4731]">Delete Article</h3>
