...
import { useUtthanCoachingRegistrations } from '@/hooks/useUtthanCoachingRegistrations';
import type { UtthanCoachingRegistrationItem } from '@/lib/types';
...
export default function RegisteredStudentsPage() {
  const { data, loading, error, deleteItem } = useUtthanCoachingRegistrations();
  const [page, setPage] = useState(1);
  const [deleteTarget, setDeleteTarget] = useState<UtthanCoachingRegistrationItem | null>(null);
            ...
            <div className="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
              <div className="max-w-2xl">
                <div className="mb-4 inline-flex items-center gap-3 rounded-full border border-white/15 bg-white/10 px-4 py-2">
                  ...
                  <span className="text-sm font-semibold tracking-[0.2em] uppercase text-white/90">Student Utthan</span>
                </div>
                <h1 className="text-3xl font-bold tracking-tight md:text-4xl">Registered Students</h1>
              </div>
            </div>
          </div>
                ...
                <table className="w-full min-w-[1800px] border-collapse">
                  <thead>
                    <tr className="bg-[linear-gradient(180deg,#f5fbf7_0%,#edf5ef_100%)] text-left">
                      {['Organization', 'Student', 'Gender', 'Mobile', 'Email', 'State', 'City', 'Course', 'Year', 'Category', 'Status', 'Date', 'Action'].map((label) => (
                  ...
                  <tbody>
                    {loading ? (
                      <tr><td colSpan={13} className="px-6 py-12 text-center text-sm text-[#6d8377]">Loading student registrations...</td></tr>
                    ) : displayRows.length === 0 ? (
                      <tr><td colSpan={13} className="px-6 py-14 text-center text-sm text-[#6d8377]">No student registrations found.</td></tr>
                    ) : (
                      paginatedRows.map((item) => (
                        <tr key={item.id} className="border-t border-[#edf2ee] text-sm text-[#2d4a3c] hover:bg-[#fbfdfb]">
                          <td className="px-4 py-4 font-semibold text-[#1f3f2f]">{item.organization_name || '-'}</td>
                          ...
                          <td className="px-4 py-4">{item.course_admission_year ?? '-'}</td>
