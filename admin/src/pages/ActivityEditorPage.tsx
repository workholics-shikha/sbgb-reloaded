...
import { ArrowLeft, Save, Upload, Zap } from 'lucide-react';
...
export default function ActivityEditorPage() {
  ...
  const { data, loading, error, addItem, updateItem, uploadImage } = useActivities();
  ...
  useEffect(() => {
    ...
    if (isEditing) {
      const activity = data.find((item) => String(item.id) === String(id));

      if (!activity) {
        setFormError('Selected activity could not be found.');
        return;
      }
  ...
  }, [categoryOptions, data, id, isEditing, loading]);
