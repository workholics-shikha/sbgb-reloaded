...
import { ChevronLeft, ChevronRight, Landmark, Trash2 } from 'lucide-react';
import { API_BASE_URL } from '@/hooks/useSliders';
import { useSbgbpRegistrations } from '@/hooks/useSbgbpRegistrations';
import type { SbgbpRegistrationItem } from '@/lib/types';
...
function paymentStatusClass(value: string) {
  ...
  if (value === 'failed' || value === 'refunded') return 'bg-red-100 text-red-700';
  return 'bg-amber-100 text-amber-700';
}

export default function RegisteredSpgbpPage() {
  const { data, loading, error, deleteItem } = useSbgbpRegistrations();
  const [page, setPage] = useState(1);
  const [deleteTarget, setDeleteTarget] = useState<SbgbpRegistrationItem | null>(null);
  ...
  async function handleDelete() {
    ...
    try {
      ...
      setSuccessMessage('SBGBP registration deleted successfully');
    } catch (deleteError) {
      setFormError(deleteError instanceof Error ? deleteError.message : 'Unable to delete registration');
    }
  }
            ...
            <div className="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
              <div className="max-w-2xl">
                <div className="mb-4 inline-flex items-center gap-3 rounded-full border border-white/15 bg-white/10 px-4 py-2">
                  ...
                  <span className="text-sm font-semibold tracking-[0.2em] uppercase text-white/90">SBGBP</span>
                </div>
                <h1 className="text-3xl font-bold tracking-tight md:text-4xl">Registered SPGBP</h1>
              </div>
            </div>
          </div>
                ...
                <table className="w-full min-w-[2200px] border-collapse">
                  <thead>
                    <tr className="bg-[linear-gradient(180deg,#f5fbf7_0%,#edf5ef_100%)] text-left">
                      {['Reg No', 'Student', 'Circle', 'Year', 'Contest', 'Class', 'Category', 'School', 'Mobile', 'Guardian', 'Payment', 'Receipt', 'Photo', 'Date', 'Action'].map((label) => (
                  ...
                  <tbody>
                    {loading ? (
                      <tr><td colSpan={15} className="px-6 py-12 text-center text-sm text-[#6d8377]">Loading SBGBP registrations...</td></tr>
                    ) : displayRows.length === 0 ? (
                      <tr><td colSpan={15} className="px-6 py-14 text-center text-sm text-[#6d8377]">No SBGBP registrations found.</td></tr>
                    ) : (
                      paginatedRows.map((item) => (
                        <tr key={item.id} className="border-t border-[#edf2ee] text-sm text-[#2d4a3c] hover:bg-[#fbfdfb]">
                          <td className="px-4 py-4 font-semibold text-[#1f3f2f]">{item.registration_no || '-'}</td>
                          <td className="px-4 py-4">{item.student_name || '-'}</td>
                          <td className="px-4 py-4">{item.circle || '-'}</td>
                          <td className="px-4 py-4">{item.reg_year ?? '-'}</td>
                          <td className="px-4 py-4">{item.contest_type || '-'}</td>
                          <td className="px-4 py-4">{item.class || '-'}</td>
                          <td className="px-4 py-4">{item.user_category || '-'}</td>
                          <td className="px-4 py-4">{item.school_name || '-'}</td>
                          <td className="px-4 py-4">{item.mobile || '-'}</td>
                          <td className="px-4 py-4">{item.mobile_guardian || '-'}</td>
                          <td className="px-4 py-4">
                            <span className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold ${paymentStatusClass(item.payment_status)}`}>
                              {(item.payment_status || 'pending').toUpperCase()} {item.payment_amount != null ? `· ₹${item.payment_amount}` : ''}
                            </span>
                          </td>
                          <td className="px-4 py-4">
                            {item.pay_receipt ? (
                              <a href={`${API_BASE_URL.replace(/\/api$/, '')}${item.pay_receipt}`} target="_blank" rel="noreferrer" className="font-semibold text-[#2e7d52] underline-offset-4 hover:underline">View Receipt</a>
                            ) : '-'}
                          </td>
                          <td className="px-4 py-4">
                            {item.student_image ? (
                              <a href={`${API_BASE_URL.replace(/\/api$/, '')}${item.student_image}`} target="_blank" rel="noreferrer" className="font-semibold text-[#2e7d52] underline-offset-4 hover:underline">View Photo</a>
                            ) : '-'}
                          </td>
