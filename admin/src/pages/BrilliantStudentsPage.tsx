...
import { useSammanSamarohRegistrations } from '@/hooks/useSammanSamarohRegistrations';
...
export default function BrilliantStudentsPage() {
  const { data, loading, error, deleteItem } = useSammanSamarohRegistrations();
  const [page, setPage] = useState(1);
  const [deleteTarget, setDeleteTarget] = useState<SammanSamarohRegistrationItem | null>(null);
            ...
            <div className="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
              <div className="max-w-2xl">
                <div className="mb-4 inline-flex items-center gap-3 rounded-full border border-white/15 bg-white/10 px-4 py-2">
                  ...
                  <span className="text-sm font-semibold tracking-[0.2em] uppercase text-white/90">Samman Samaroh</span>
                </div>
                <h1 className="text-3xl font-bold tracking-tight md:text-4xl">Brilliant Students</h1>
                <p className="mt-3 max-w-2xl text-sm text-white/80">
                  Public samman-samaroh registrations are listed here for review.
                </p>
              </div>
            </div>
          </div>
                ...
                <table className="w-full min-w-[1800px] border-collapse">
                  <thead>
                    <tr className="bg-[linear-gradient(180deg,#f5fbf7_0%,#edf5ef_100%)] text-left">
                      {['Student', 'Father', 'Mobile', 'Session', 'Class/Course', 'Marks %', 'State', 'City', 'Board', 'Document', 'Date', 'Action'].map((label) => (
                  ...
                  <tbody>
                    {loading ? (
                      <tr><td colSpan={12} className="px-6 py-12 text-center text-sm text-[#6d8377]">Loading registrations...</td></tr>
                    ) : displayRows.length === 0 ? (
                      <tr><td colSpan={12} className="px-6 py-14 text-center text-sm text-[#6d8377]">No registrations found.</td></tr>
                    ) : (
                      paginatedRows.map((item) => (
                        <tr key={item.id} className="border-t border-[#edf2ee] text-sm text-[#2d4a3c] hover:bg-[#fbfdfb]">
                          <td className="px-4 py-4 font-semibold text-[#1f3f2f]">{item.student_name || '-'}</td>
                          <td className="px-4 py-4">{item.father_name || '-'}</td>
                          <td className="px-4 py-4">{item.mobile || '-'}</td>
                          <td className="px-4 py-4">{item.academic_session || '-'}</td>
                          <td className="px-4 py-4">{item.class_course_degree || '-'}</td>
                          <td className="px-4 py-4">{item.marks_percentage ?? '-'}</td>
                          <td className="px-4 py-4">{item.state_name || '-'}</td>
                          <td className="px-4 py-4">{item.city_name || '-'}</td>
                          <td className="px-4 py-4">{item.board_university || '-'}</td>
                          <td className="px-4 py-4">
                            {item.result_document ? (
                              <a
                                href={`${API_BASE_URL.replace(/\/api$/, '')}${item.result_document}`}
                                target="_blank"
                                rel="noreferrer"
                                className="font-semibold text-[#2e7d52] underline-offset-4 hover:underline"
                              >
                                View File
                              </a>
                            ) : '-'}
