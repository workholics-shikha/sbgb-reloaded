...
import { ChevronLeft, ChevronRight, ImagePlus, BookOpenText, Pencil, Trash2, Upload, X } from 'lucide-react';
...
function resolveImageUrl(imagePath?: string | null) {
  ...
  return `${uploadsBaseUrl}/uploads/stories/${imagePath}`;
}

export default function StoriesPage() {
  const { data, loading, error, addItem, updateItem, deleteItem, uploadImage } = useStories();
  ...
  const [formValues, setFormValues] = useState<StoryFormValues>({
    title: '',
    image: '',
    story_place: '',
    story_date: '',
  ...
  function openAdd() {
    setEditingItem(null);
    setFormValues({ title: '', image: '', story_place: '', story_date: '', description: '', status: '1' });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const payload = {
        ...
        image: imagePath,
        story_place: formValues.story_place.trim(),
        story_date: formValues.story_date.trim(),
      ...
      setFormError(submitError instanceof Error ? submitError.message : 'Unable to save story');
    } finally {
      setSaving(false);
    }
  }
  ...
  async function handleDelete() {
    ...
    try {
      ...
      setFormError(deleteError instanceof Error ? deleteError.message : 'Unable to delete story');
    }
  }
            ...
            <div className="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
              <div className="max-w-2xl">
                <div className="mb-4 inline-flex items-center gap-3 rounded-full border border-white/15 bg-white/10 px-4 py-2">
                  <span className="flex h-10 w-10 items-center justify-center rounded-full bg-[#e8a317] text-[#1a4731]"><BookOpenText size={18} /></span>
                  <span className="text-sm font-semibold tracking-[0.2em] uppercase text-white/90">Story Management</span>
                </div>
                <h1 className="text-3xl font-bold tracking-tight md:text-4xl">Stories</h1>
              </div>
                ...
                <table className="w-full min-w-[1180px] border-collapse">
                  <thead>
                    <tr className="bg-[linear-gradient(180deg,#f5fbf7_0%,#edf5ef_100%)] text-left">
                      {['Image', 'Title', 'Place', 'Date', 'Status', 'Actions'].map((label) => (
                  ...
                  <tbody>
                    {loading ? (
                      <tr><td colSpan={6} className="px-6 py-12 text-center text-sm text-[#6d8377]">Loading stories...</td></tr>
                    ) : displayRows.length === 0 ? (
                      <tr><td colSpan={6} className="px-6 py-14 text-center text-sm text-[#6d8377]">No stories found.</td></tr>
                    ) : (
                      paginatedRows.map((item) => (
                        <tr key={item.id} className="border-t border-[#edf2ee] text-sm text-[#2d4a3c] hover:bg-[#fbfdfb]">
                          <td className="px-4 py-4">{item.image ? <img src={resolveImageUrl(item.image)} alt={item.title || 'Story image'} className="h-16 w-16 rounded-xl border border-[#dfe9e3] bg-white object-cover" /> : 'No image'}</td>
                          <td className="px-4 py-4 font-semibold text-[#1f3f2f]">{item.title || '-'}</td>
                          <td className="px-4 py-4">{item.story_place || '-'}</td>
                          <td className="px-4 py-4">{item.story_date || '-'}</td>
          ...
          <div className="w-full max-w-4xl overflow-hidden rounded-[28px] border border-[#d7e4db] bg-white shadow-[0_35px_80px_rgba(8,30,20,0.3)]">
            <div className="bg-[linear-gradient(135deg,#1a4731_0%,#2e7d52_100%)] px-6 py-5 text-white">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#f0cd72]">Story Form</p>
                  <h2 className="mt-1 text-xl font-semibold">{editingItem ? 'Edit Story' : 'Add New Story'}</h2>
                </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5 px-6 py-6">
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div>
                  <label className="mb-1.5 block text-sm font-medium text-[#456353]">Place</label>
                  <input required value={formValues.story_place} onChange={(e) => setFormValues((current) => ({ ...current, story_place: e.target.value }))} className="w-full rounded-2xl border border-[#d7e4db] px-4 py-3 text-sm text-[#1a4731] outline-none" />
                </div>
              </div>

              <div className="grid gap-4 md:grid-cols-2">
                <div>
                  <label className="mb-1.5 block text-sm font-medium text-[#456353]">Date</label>
                  <input required value={formValues.story_date} onChange={(e) => setFormValues((current) => ({ ...current, story_date: e.target.value }))} className="w-full rounded-2xl border border-[#d7e4db] px-4 py-3 text-sm text-[#1a4731] outline-none" />
                </div>
                <div>
              ...
              {(selectedImageFile || formValues.image) && previewUrl && <img src={previewUrl} alt="Story preview" className="h-52 w-full rounded-2xl border border-[#e4ece7] bg-white object-cover" />}

              <div>
                <label className="mb-1.5 block text-sm font-medium text-[#456353]">Description</label>
                <textarea required value={formValues.description} onChange={(e) => setFormValues((current) => ({ ...current, description: e.target.value }))} rows={8} className="w-full rounded-2xl border border-[#d7e4db] px-4 py-3 text-sm text-[#1a4731] outline-none" />
              </div>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button type="submit" disabled={saving} className="flex-1 rounded-2xl bg-[linear-gradient(135deg,#1a4731_0%,#2e7d52_100%)] px-4 py-3 text-sm font-semibold text-white disabled:opacity-60">{saving ? 'Saving...' : editingItem ? 'Update Story' : 'Save Story'}</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#0b1e15]/60 p-4 backdrop-blur-sm">
          <div className="w-full max-w-sm rounded-[28px] border border-[#d7e4db] bg-white p-6 shadow-[0_35px_80px_rgba(8,30,20,0.28)]">
            ...
            <h3 className="text-lg font-semibold text-[#1a4731]">Delete Story</h3>
            <p className="mt-2 text-sm leading-6 text-[#61766a]">This will remove <span className="font-semibold text-[#1f3f2f]">{deleteTarget.title}</span> from the stories table.</p>
